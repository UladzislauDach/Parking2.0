...
class Parking {
    ...
    public void addCar(ArrayList<Car> carListWithFile) {
        for (Car car : carListWithFile) {
            if (validationCar(car)) {
                carList.add(car);
                numberPlaces = numberPlaces - 1;
                System.out.println(car.toString() + " приехал на стоянку" +
                ...
                System.out.println(car.toString() + " не может посетить стоянку (нет мест, с прицепом, большой вес)." +
                        " Свободных мест : " + numberPlaces);
        }
    }
